...
def test_download_summary_pdf_handles_markdown_unicode_tables_and_long_text(owner):
    long_token = "A" * 5000
    summary = (
        "# Executive Summary\n\n"
        "UTF-8 text: café, 你好, Привет, مرحبا.\n\n"
        "- Bullet one\n"
        "- [ ] Action item\n"
        "1. Ordered item\n\n"
        "| Owner | Detail |\n"
    ...
    with _db():
        u = db.session.get(User, owner)
        rid = make_recording(u, summary=summary, title="PDF Export Smoke").id

    c = new_client()
    login(c, owner)
    resp = c.get(f"/recording/{rid}/download/summary?format=pdf")
    assert resp.status_code == 200
...
def test_download_summary_txt_format_still_works(owner):
    with _db():
        u = db.session.get(User, owner)
        rid = make_recording(u, summary="## Heading\n\nSome summary content.", title="TXT Export").id

    c = new_client()
    login(c, owner)
    resp = c.get(f"/recording/{rid}/download/summary?format=txt")
    assert resp.status_code == 200
    assert resp.headers["Content-Type"].startswith("text/plain")
    assert "Some summary content." in resp.data.decode("utf-8")


def test_download_summary_unsupported_formats_remain_rejected(owner):
    with _db():
        u = db.session.get(User, owner)
        rid = make_recording(u, summary="Summary content.", title="Unsupported Export").id

    c = new_client()
    login(c, owner)
    for export_format in ("json", "md"):
        resp = c.get(f"/recording/{rid}/download/summary?format={export_format}")
        assert resp.status_code == 400
