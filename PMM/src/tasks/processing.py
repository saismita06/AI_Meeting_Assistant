...
def generate_title_task(app_context, recording_id, will_auto_summarize=False):
    ...
    with app_context:
        ...
        if not needs_ai_title:
            ...
            current_app.logger.warning(f"Skipping AI title generation for {recording_id}: text model client not configured.")
            ai_title = None
...
def generate_summary_only_task(app_context, recording_id, custom_prompt_override=None, custom_prompt_append=False, user_id=None):
    ...
    with app_context:
        ...
        if client is None:
            current_app.logger.warning(f"Skipping summary generation for {recording_id}: text model client not configured.")
            recording.summary = "[Summary skipped: text model client not configured]"
            db.session.commit()
            return
        ...
        current_app.logger.info(f"Requesting summary from configured text model for recording {recording_id} using model {TEXT_MODEL_NAME}...")
    ...
    with app_context:
        ...
        try:
            ...
            if not will_auto_summarize:
                ...
                # Skip summary generation if this recording already has a summary.
                if recording and recording.summary:
                    current_app.logger.info(
                        f"Summary already exists for recording {recording_id}, skipping PMM summary generation"
                    )
                    ...
                    # Generate summary using PMM LLM
                    current_app.logger.info(
                        f"Auto-generating summary for recording {recording_id}"
                    )
