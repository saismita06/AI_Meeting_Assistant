...
class ConnectorRegistry:
    ...
    def initialize_from_env(self) -> BaseTranscriptionConnector:
        ...
        if not connector_name:
            ...
            transcription_base_url = os.environ.get('TRANSCRIPTION_BASE_URL', '').strip()
            ...
            if asr_base_url or self._is_self_hosted_asr_endpoint(transcription_base_url) or use_asr_flag:
                connector_name = 'asr_endpoint'
                if asr_base_url:
                    logger.info("Auto-detected ASR endpoint from ASR_BASE_URL")
                elif transcription_base_url:
                    logger.info("Auto-detected ASR endpoint from TRANSCRIPTION_BASE_URL")
    ...
    def _is_self_hosted_asr_endpoint(self, base_url: str) -> bool:
        """Treat non-cloud transcription URLs as ASR endpoints."""
        if not base_url:
            return False
        normalized = base_url.split('#')[0].strip().lower()
        cloud_markers = (
            'api.openai.com',
            'api.mistral.ai',
            '.openai.azure.com',
        ...
        return normalized.startswith(('http://', 'https://')) and not any(
            marker in normalized for marker in cloud_markers
        )

    def _normalize_asr_base_url(self, base_url: str) -> str:
        """Convert OpenAI-compatible ASR URLs to the service root used by /asr."""
        cleaned = base_url.split('#')[0].strip().rstrip('/')
    ...
    def _build_config_from_env(self, connector_name: str) -> Dict[str, Any]:
        ...
        if connector_name == 'asr_endpoint':
            base_url = os.environ.get('ASR_BASE_URL', '') or os.environ.get('TRANSCRIPTION_BASE_URL', '')
            if base_url:
                base_url = self._normalize_asr_base_url(base_url)
