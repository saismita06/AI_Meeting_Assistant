...
from src.services.summary_export import (
    build_summary_export_data,
    render_summary_docx,
    render_summary_pdf,
    render_summary_txt,
)
...
def download_summary_word(recording_id):
    """Download recording summary as DOCX, PDF, or TXT."""
    try:
        recording = db.session.get(Recording, recording_id)
        if not recording:
            return jsonify({'error': 'Recording not found'}), 404
        ...
        safe_base = f'summary-{safe_title}' if safe_title else f'summary-recording-{recording_id}'
        export_format = request.args.get('format', 'docx').lower()
        if export_format not in {'docx', 'pdf', 'txt'}:
            return jsonify({'error': 'Unsupported summary download format'}), 400

        export_data = build_summary_export_data(recording, current_user)

        if export_format == 'txt':
            response = make_response(render_summary_txt(export_data))
            response.headers['Content-Type'] = 'text/plain; charset=utf-8'
            filename = f'{safe_base}.txt'
        elif export_format == 'pdf':
            response = send_file(
                render_summary_pdf(export_data),
                as_attachment=False,
            ...
            response = send_file(
                render_summary_docx(export_data),
                as_attachment=False,
        ...
        try:
            ...
            try:
                response.headers['Content-Disposition'] = f"attachment; filename*={encode_rfc2231(filename, charset='utf-8')}"
            except Exception as e:
                current_app.logger.error(f"RFC2231 encoding failed: {e}, using fallback")
                response.headers['Content-Disposition'] = f'attachment; filename="summary-recording-{recording_id}.{export_format}"'
        return response

    except Exception as e:
        current_app.logger.error(f"Error generating summary download: {e}", exc_info=True)
        return jsonify({'error': 'Failed to generate summary download'}), 500
...
def generate_summary_endpoint(recording_id):
    ...
    try:
        ...
        # Check if text model client is available
        if client is None:
            return jsonify({'error': 'Summary service is not available (text model client not configured)'}), 503
...
def regenerate_title(recording_id):
    ...
    try:
        ...
        if client is None:
            return jsonify({'error': 'Title generation service is not available (text model client not configured)'}), 503
