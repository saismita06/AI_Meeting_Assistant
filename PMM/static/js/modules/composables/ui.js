...
import { formatSummaryAsPlainText } from '../utils/summary.js';
...
export function useUI(state, utils, processedTranscription) {
    const {
        ...
        recordingNotes, showDownloadMenu, showSummaryDownloadMenu, currentPlayingSegmentIndex, followPlayerMode,
    ...
    const copySummary = (event) => {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            const summaryText = formatSummaryAsPlainText(selectedRecording.value.summary, selectedRecording.value);
            navigator.clipboard.writeText(summaryText)
                .then(() => showToast(t('messages.summaryCopied')))
                .catch(() => fallbackCopyTextToClipboard(summaryText));
        } else {
            fallbackCopyTextToClipboard(formatSummaryAsPlainText(selectedRecording.value.summary, selectedRecording.value));
        }
    };
    ...
    // --- Download Functions ---
    const downloadSummary = async (format = 'docx') => {
        if (!selectedRecording.value || !selectedRecording.value.summary) {
        ...
        try {
            const requestedFormat = ['pdf', 'docx', 'txt'].includes(String(format).toLowerCase())
                ? String(format).toLowerCase()
                : 'docx';
            const response = await fetch(`/recording/${selectedRecording.value.id}/download/summary?format=${requestedFormat}`);
            if (!response.ok) {
                const error = await response.json();
    ...
    const setupGlobalClickHandler = () => {
        document.addEventListener('click', (event) => {
            ...
            if (showSummaryDownloadMenu && showSummaryDownloadMenu.value) {
                const summaryDownloadButton = target.closest('[data-summary-download-toggle]');
                const summaryDownloadDropdown = target.closest('[data-summary-download-dropdown]');

                if (!summaryDownloadButton && !summaryDownloadDropdown) {
                    showSummaryDownloadMenu.value = false;
                }
            }
